...
// db()->connect(['dbname' => 'myshopos_gafi2', 'password' => '0503941618']);
db()->autoConnect();
...
app()->get("/about", function () {
    response()->json(["name" => "About"]);
});

app()->get("/items", function () {
    // response()->json(db()->select('stock')->search("item", "water"));
    $user = auth()->user();
    if ($user) {
        // user is logged in
        $res = db()->insert('stock')->params(["cat_id" => 1, "item" => "MARY KAY LIQUID EYE SHADOW", "stores" => '[]', "photos" => '[]', "emp_id" => 1])->unique('item')->execute();
        if ($res === false) {
